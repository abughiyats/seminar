...
class Riwayat_Seminar extends REST_Controller {	
	...
	public function index_GET(){
		...
		if(!empty($this->input->get('m_pfx'))){
			if($this->input->get('m_pfx')=="form"){
				$this->load->view('menu/peserta/riwayat-seminar/form.php',$data);
				return;
			}
		}
		$this->load->view('menu/peserta/riwayat-seminar/view.php',$data);
		return;
	}
	...
	public function index_POST(){
		...
		print $this->load->view('menu/peserta/riwayat-seminar/form.php',$data);
	}
	...
	public function ajax_list_GET(){
		if(!empty($this->input->get('riwayat_seminar_code'))){$where = array($this->riwayat_seminar_model->column_search[0] => $this->input->get('riwayat_seminar_code'));
			...
			$where = "1=1 AND mn_seminar_detail.profile_code = '".$this->session->userdata(NAME_SESSION)['user_profile']."' AND mn_seminar.target_date <= DATE(NOW())";
		}
		...
		foreach ($result as $seminar_list) {
        	$edit_seminar = "'".base_url('src/menu/peserta/Riwayat_Seminar')."?m_pfx=form&code=".$seminar_list->riwayat_seminar_code."'";
			...
			$row[] = number_format($seminar_list->riwayat_seminar_payment_amount,2);
			$row[] = $seminar_list->riwayat_seminar_certificate;
