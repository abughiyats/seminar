...
class Jadwal_Seminar extends REST_Controller {	
	var $alias_table = 'seminar_';
	public function __construct(){
		parent::__construct();
		$this->load->model('menu/Jadwal_Seminar_Model','jadwal_seminar_model');
		$this->load->model('util/Function_Model','fn');
        $this->load->model('security/Gateway_Model','gateway_model');
	}

	public function index_GET(){
        $mn_tambah = "'".base_url('src/menu/jadwal-seminar')."?m_pfx=form'";
        $mn_cancel = "'".base_url('src/menu/jadwal-seminar')."?m_pfx=view'";
        $mn = "'".base_url('src/menu/jadwal-seminar')."?m_pfx=form'";
		...
		if(!empty($this->input->get('m_pfx'))){
			if($this->input->get('m_pfx')=="form"){
				$this->load->view('menu/jadwal-seminar/form.php',$data);
				return;
			}
		}
		$this->load->view('menu/jadwal-seminar/view.php',$data);
		return;
	}
	...
	public function index_POST(){
		...
		$data = $this->jadwal_seminar_model->get_One($where);
		print $this->load->view('menu/jadwal-seminar/form.php',$data);
	}

	public function ajax_list_GET(){
		if(!empty($this->input->get('seminar_code'))){$where = array($this->jadwal_seminar_model->column_search[0] => $this->input->get('seminar_code'));
		}else{$where = "1=1";}

		$result = $this->jadwal_seminar_model->get_datatables($where);
		$data = array();
		$no = $this->input->get('seminar_start');
		foreach ($result as $seminar_list) {
        	$edit_seminar = "'".base_url('src/menu/jadwal-seminar')."?m_pfx=form&code=".$seminar_list->seminar_code."'";
			...
			$row[] = $seminar_list->seminar_company_code;
			...
			$row[] = date("d-m-Y", strtotime($seminar_list->seminar_open_regist_date));
		...
		$data = array(
				...
				, "recordsTotal" => $this->jadwal_seminar_model->get_count($where)
				, "recordsFiltered" => $this->jadwal_seminar_model->count_filtered($where)
				, "data" => $data
				, "message"	=> $message
				, "status" 	=> $status
			);
	...
	public function reqs_one_GET(){
		if(!empty($this->input->get('code'))){
			$where = $this->jadwal_seminar_model->column_search[0]." = '".$this->input->get('code')."' ";
		}else{
			$where = "";
		}

		$result = $this->jadwal_seminar_model->get_One($where);
	...
	public function reqs_list_GET(){
		...
		$result = $this->jadwal_seminar_model->get_list($where);
	...
	public function insert_POST() {	
		...
		$result = $this->jadwal_seminar_model->insert($data);
	...
	public function update_POST(){
		...
		$result = $this->jadwal_seminar_model->update($data, array($this->jadwal_seminar_model->column_search[0] => $this->input->post('seminar_code')));
	...
	public function delete_GET(){
		...
		$result = $this->jadwal_seminar_model->delete($where);

		if (!$result){$message = $this->fn->get_message($this->input->get('seminar_code'), DELETE, FALSE); $status = FALSE;
		}else{$message = $this->fn->get_message($this->input->get('seminar_code'), DELETE, TRUE);  $status = TRUE;}
